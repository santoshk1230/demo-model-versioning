import re
# import inflection
# from src.score_generator import ScoreGenerator as sg

# from model.Pinelabs_2_0_0 import Pinelabs_2_0_0 as rd
# rule_dict = rd.get_rules()

def get_client_klass2(payload: dict):
    client_name = payload["client_name"]
    m_version = payload["m_version"]
    client_klass_name = f"{client_name.capitalize()}_{str(m_version).replace('.', '_')}"
    module = __import__(
        f"model.{client_klass_name}",
        fromlist=[client_klass_name],
    )
    client_klass = getattr(module, client_klass_name)
    return client_klass
...
def get_failed_trigger_range(x, ind_triggers, score_to_cols, min_max_scores2):
...
def get_confidence_score(transformed_payload):
    # client_klass = sg.get_client_klass(transformed_payload)
    # score_gen = sg(data={}, payload=transformed_payload)
    client_klass = get_client_klass2(transformed_payload)
    rule_dict = client_klass().rules
    ...
    for k,v in score_to_cols.items():
        v = [v1 for v1 in v if not isinstance(v1,dict)]
        score_to_cols[k] = list(set(v))
    ...
    failed_trigger_range = get_failed_trigger_range(x4, ind_triggers, score_to_cols, min_max_scores2)
